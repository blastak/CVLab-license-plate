...
def homography_quality_score():
    pass


def save_annotation_to_json():
    pass
...
if __name__ == '__main__':
    ...
    for _, img_path in enumerate(img_paths):
        ...
        boxes = []
        # XML 값 저장
        bb_xml = BBox(left, top, right - left, bottom - top, plate_number, int(plate_type[1:]) - 1, 1.0)
        boxes.append(bb_xml)

        # # VIN_LPD로 검출
        # d_out = d_net.resize_N_forward(img)
        # for _, d in enumerate(d_out):
        #     bb_vinlpd = BBox(d.x, d.y, d.w, d.h, '', d.class_idx, d.conf)
        ...
        for _, bb_or_qb in enumerate(boxes):
            ...
            # 좌표 계산
            src_xy = np.array([[0, 0, 1], [g_w, 0, 1], [g_w, g_h, 1], [0, g_h, 1]])
            dst_xy = mat_T @ src_xy.T
            dst_xy /= dst_xy[-1, :]  # normalization
            dst_xy = dst_xy[:2, :].T  # transpose, crop
